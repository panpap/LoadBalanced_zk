...
public class ZooKeeperMain {
    ...
    //XXX panpap was here
    protected void connectToZK(String newHost,String path) throws InterruptedException, IOException {
        host = newHost;
        System.out.println("\n\nCONNECTTOZK\n\n");
        boolean readOnly = cl.getOption("readonly") != null;  
        //panpap: make new connection
        ZooKeeper tempzk = new ZooKeeper(host,
                 Integer.parseInt(cl.getOption("timeout")),
                 new MyWatcher(), readOnly);
        //panpap: make new watcher
      /*  if (path!=null)
        {
        	try {
				tempzk.getData(path, true, new Stat());
			} catch (KeeperException e) {
				e.printStackTrace();
			}
        }*/
        //panpap: close previous connection
    ...
    private class MyWatcher implements Watcher {
        public void process(WatchedEvent event) {
        	if (event.getType()!=EventType.MoveToServer)
        	{
        		if (getPrintWatches()) {
	        ...
	        {
	        	String msg=event.getPath();
	        	System.out.println(msg);
	        	String[] parts=msg.split("#");
	        	//panpap: set again the Watcher immediately
	        	LOG.info("panpap: Re-setting watcher on "+parts[0]+" due to load balancer");
	        	try {
					zk.getData(parts[0], true, new Stat());
				} catch (KeeperException e) {
					e.printStackTrace();
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
	        	//panpap: try to connect to the other server
	        	LOG.info("panpap: Moving to server "+parts[1]+" due to load balancer");
	        	try {
	        		connectToZK(parts[1].split("/")[1],parts[0]);
				} catch (InterruptedException e) {
					e.printStackTrace();
				} catch (IOException e) {
					e.printStackTrace();
